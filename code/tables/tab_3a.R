...
make_tab_3a <- function(data = NULL, root = study_root()) {
  ...
  html_path <- output_path("outputs", "tab_3a.html", root = root)
  ...
  part1 <- capture(
    print(table1::table1(
      ...
      data = dplyr::filter(data, Vignette == "Deportation"),
      render = "Mean (SD)"
    ))
  )
  ...
  html <- paste0(
    ...
    "<title>Table 3a</title></head><body>",
    "<h1>Table 3a — Summary statistics, Deportation vignette</h1>",
    "<pre>", paste(part1, collapse = "\n"), "</pre>",
    "</body></html>"
  )

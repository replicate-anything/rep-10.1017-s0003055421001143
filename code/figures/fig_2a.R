...
make_fig_2a <- function(data = NULL, root = study_root()) {
  if (is.null(data)) data <- load_analysis_data()
  labels <- outcome_labels()

  dfcoef <- data |>
    dplyr::select(D1Agree:D3Agree, caseOutcome, judgment, Vignette, stratum) |>
  ...
  p <- ggplot2::ggplot(
    ...
    ggplot2::ggtitle("Effect of EC disagreeing with domestic court")

  out <- output_path("outputs", "fig_2a.png", root = root)
  ensure_dir(dirname(out))
  ggplot2::ggsave(out, plot = p, height = 4, width = 6, units = "in")
  p
}

format_fig_2a <- function(object) object

if (sys.nframe() == 0L) make_fig_2a()

...
make_fig_2c <- function(data = NULL, root = study_root()) {
  ...
  p <- ggplot2::ggplot(
    ...
    ggplot2::ggtitle("Effect of EC disagreeing with domestic court by country (averaging over vignettes)")

  out <- output_path("outputs", "fig_2c.png", root = root)
  ensure_dir(dirname(out))
  ggplot2::ggsave(out, plot = p, height = 5, width = 10, units = "in")
  p
}

format_fig_2c <- function(object) object

if (sys.nframe() == 0L) make_fig_2c()

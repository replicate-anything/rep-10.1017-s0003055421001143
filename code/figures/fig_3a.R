...
make_fig_3a <- function(data = NULL, root = study_root()) {
  if (is.null(data)) data <- load_analysis_data()
  labels <- outcome_labels()

  dfcoef <- data |>
    dplyr::select(D1Agree:D3Agree, caseOutcome, judgment, VignetteOutcome, Vignette, SatLaw, LGBTRights, stratum) |>
  ...
  out <- output_path("outputs", "fig_3a.png", root = root)
  ensure_dir(dirname(out))
  ggplot2::ggsave(out, plot = p, height = 6, width = 6, units = "in")
  p
}

format_fig_3a <- function(object) object

if (sys.nframe() == 0L) make_fig_3a()
